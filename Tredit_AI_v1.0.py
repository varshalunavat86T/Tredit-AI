...
st_autorefresh(interval=1000, limit=None, key="tredit_ai_big_candle_v15")

st.set_page_config(page_title="Tredit AI Master Engine", page_icon="🟨", layout="wide")

# CSS Styling (Large Candles + Compact Controls)
st.markdown("""
    <style>
    .stApp { background-color: #0b0d10; color: #ffffff; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
    
    h1 { font-size: 18px !important; margin: 0px !important; }
    h2 { font-size: 15px !important; margin: 0px !important; }
    h3 { font-size: 13px !important; margin: 0px !important; }
    p { font-size: 11px !important; margin: 1px 0px !important; }
    ...
    .solid-candle-compact h2 { color: #000000 !important; font-size: 13px !important; font-weight: 900 !important; margin: 0 !important; }
    .solid-candle-compact h4 { color: #000000 !important; font-size: 10px !important; margin: 0 !important; }
    </style>
""", unsafe_allow_html=True)

# 📊 1. FETCH REAL DATA WITH TICK ENGINE SIMULATION
@st.cache_data(ttl=1)
...
base_spot, chart_df = fetch_exact_banknifty_data()

# ⏱️ 2. LIVE TICKING LOGIC (ताकि बाज़ार बंद में भी रेट्स 1-1 सेकंड में हिलते रहें)
curr_sec_time = int(time.time())
random.seed(curr_sec_time)

# 1-सेकंड का लाइव माइक्रो टिक
tick_fluctuation = round(random.uniform(-3.5, 3.5), 2)
spot_val = round(base_spot + tick_fluctuation, 2)
...
master_power = round(84.0 + random.uniform(-2.5, 3.5), 1)
...
with col_h2:
    st.markdown(f"<p style='text-align:right; color:#AAAAAA;'>LIVE TICK TIME: <b>{curr_time_str}</b></p>", unsafe_allow_html=True)
...
# ⏰ 4. EXACT TIME BUY SIGNAL ALERT
st.markdown(f"""
<div class="time-alert-box">
    <h2 style="margin:0; font-size:16px;">🟢 AI ENTRY SIGNAL: {master_power}% CONFIRMED</h2>
    <h3 style="margin:2px 0; font-size:15px; color:#FFFFFF;">👉 BUY CALL (CE) EXACTLY AT <span style="color:#FFD700; background:#000; padding:2px 6px; border-radius:4px;">{next_min_time_str}</span> (TARGET +35 PTS)</h3>
</div>
""", unsafe_allow_html=True)

# ⏱️ 5. TIMER & DYNAMIC TARGET
...
# 📊 6. LARGE HD CANDLESTICK CHART (बड़ी और साफ़ कैंडल्स)
st.markdown("<p style='margin-top:4px; font-weight:bold; color:#00E676;'>📈 HD LIVE CANDLESTICK CHART (LARGE VIEW)</p>", unsafe_allow_html=True)
...
if chart_df is not None and not chart_df.empty:
    ...
    fig.update_layout(
        ...
        height=450, # बड़ी कैंडल्स के लिए हाइट बढ़ाई गई
...
# 🚀 7. 8 BUDGET STRIKE OPTIONS
st.markdown("<p style='font-weight:bold; color:#FFD700; margin-top:2px;'>🚀 8 BUDGET STRIKE OPTIONS (₹50 TO ₹400)</p>", unsafe_allow_html=True)
...
for idx, b in enumerate(budget_ranges):
    ...
    curr_prem = round(b["base_p"] + (spot_val % 20) * 0.5 + random.uniform(-1.0, 1.0), 1)
    ...
    <div class="strike-card-compact">
        ...
        <p style="margin:1px 0;"><b>Rate:</b> ₹{curr_prem}</p>
...
st.markdown("<p style='font-weight:bold; color:#00E676; margin-top:2px;'>🕯️ 8 MASTER SYSTEM MODULES</p>", unsafe_allow_html=True)
m_cols = st.columns(8)
